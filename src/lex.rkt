...
(define start-lex (lambda (filename) (set! file-port (open-input-file filename))))

; close the input file

(define end-lex (lambda () (close-input-port file-port)))

; the current list of reserved words and operator characters

(define reserved-word-list
  '(if else
       return
       while
       break
...
(define return-num-lex (lambda (value) (cons 'NUMBER value)))

(define return-symbol-lex (lambda (symbol) (cons 'BINARY-OP symbol)))

;(define return-add-lex
;  (lambda (symbol)
;    (cons 'MATHOP symbol)))

(define return-left-paren-lex (lambda () (cons 'LEFTPAREN '())))

(define return-right-paren-lex (lambda () (cons 'RIGHTPAREN '())))

(define return-assign-lex (lambda (symbol) (cons 'ASSIGN symbol)))

(define return-null-lex (lambda (symbol) (cons 'UNKNOWN symbol)))

(define return-semicolon-lex (lambda () (cons 'SEMICOLON '())))

(define return-leftbrace-lex (lambda () (cons 'LEFTBRACE '())))

(define return-rightbrace-lex (lambda () (cons 'RIGHTBRACE '())))

(define return-comma-lex (lambda () (cons 'COMMA '())))

(define return-eof-lex (lambda () (cons 'EOF '())))

(define return-period-lex (lambda () (cons 'BINARY-OP 'dot)))
    ...
    (let ([nextchar (readchar file-port)])
      (cond
        ...
        [(char-alphabetic? nextchar)
         (return-id-lex (string->symbol (id-lex file-port (make-string 1 nextchar))))]
        [(char-numeric? nextchar) (return-num-lex (num-lex file-port (addtointeger 0 nextchar)))]
        [(memq nextchar reserved-operator-list)
         (return-symbol-lex (string->symbol (symbol-lex file-port (make-string 1 nextchar))))]
  ...
  (lambda (fport idstring)
    (let ([nextchar (readchar fport)])
      (if (or (char-alphabetic? nextchar) (char-numeric? nextchar) (char=? #\_ nextchar))
          ...
          (begin
            (unreadchar nextchar fport)
            idstring)))))

(define addtointeger
  (lambda (val nextdigit) (+ (* val 10) (- (char->integer nextdigit) (char->integer #\0)))))
