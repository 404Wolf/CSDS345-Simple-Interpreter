...
(define program-parse
  (lambda ()
    (if (eq? (car (get-next-symbol)) 'EOF)
        ...
        (begin
          (unget-next-symbol)
          (let ([parsetree (class-parse)]) (cons parsetree (program-parse)))))))
    ...
    (let ([firstsymbol (get-next-symbol)])
      (cond
        ...
        [(eq? (car firstsymbol) 'ID)
         (cond
           [(not (equal? (cdr firstsymbol) classname))
            (error 'parser "Illegal language structure or constructor with different name as class")]
           [(not (eq? (car (get-next-symbol)) 'LEFTPAREN))
            (error 'parser "Illegal start of constructor definition")]
...
(define simple-statement-parse
  (lambda ()
    (let ([nextsymbol (get-next-symbol)]
      ...
      (begin
        (cond
          [(eq? (car nextsymbol) 'return) (set! parse-statement (return-parse))]
          ...
          [(eq? (car nextsymbol) 'ID)
           (begin
             (unget-next-symbol)
             (set! parse-statement (function-or-assign-parse)))]
...
(define compound-statement-parse
  (lambda ()
    (if (eq? (car (get-next-symbol)) 'RIGHTBRACE)
        ...
        (begin
          (unget-next-symbol)
          (let ([s (statement-parse)]) (cons s (compound-statement-parse)))))))

; parse a return statement: return followed by a value.

(define return-parse (lambda () (list 'return (value-parse))))
    ...
    (let ([nextsymbol (car (get-next-symbol))])
      (if (not (eq? nextsymbol 'catch))
          ...
          (let* ([firstsymbol (get-next-symbol)]
            ...
            (cond
              [(not (eq? (car firstsymbol) 'LEFTPAREN)) (error 'parser "Missing left parenthesis")]
              [(not (eq? (car secondsymbol) 'ID)) (error 'parser "Missing exception parameter")]
              [(not (eq? (car thirdsymbol) 'RIGHTPAREN))
               (error 'parser "Missing closing parenthesis")]
...
(define funcall-parse (lambda (name) (cons 'funcall (cons name (get-actualparameter-list)))))
...
(define value-parse (lambda () (assignterm-parse (get-next-symbol))))
...
(define assignterm-parse (lambda (firstsymbol) (assignterm-parse-helper (orterm-parse firstsymbol))))
...
(define orterm-parse (lambda (firstsymbol) (orterm-parse-helper (andterm-parse firstsymbol))))
...
(define andterm-parse (lambda (firstsymbol) (andterm-parse-helper (equalterm-parse firstsymbol))))
...
(define equalterm-parse
  (lambda (firstsymbol) (equalterm-parse-helper (compareterm-parse firstsymbol))))
...
(define compareterm-parse
  (lambda (firstsymbol) (compareterm-parse-helper (addterm-parse firstsymbol))))
  ...
  (lambda (firstoperand)
    (let ([op (get-next-symbol)])
      (if (and (eq? (car op) 'BINARY-OP)
               (or (eq? (cdr op) '<) (eq? (cdr op) '<=) (eq? (cdr op) '>) (eq? (cdr op) '>=)))
...
(define addterm-parse (lambda (firstsymbol) (addterm-parse-helper (multterm-parse firstsymbol))))
...
(define multterm-parse (lambda (firstsymbol) (multterm-parse-helper (operand-parse firstsymbol))))
...
(define operand-parse
  (lambda (firstsymbol)
    (cond
      ...
      [(and (eq? (car firstsymbol) 'BINARY-OP) (eq? (cdr firstsymbol) '-))
       (list '- (operand-parse (get-next-symbol)))] ; this is a new line
      [(and (eq? (car firstsymbol) 'BINARY-OP) (eq? (cdr firstsymbol) '!))
       (list '! (operand-parse (get-next-symbol)))] ; this is a new line
      ...
      [(eq? (car firstsymbol) 'new)
       (let* ([classid (get-next-symbol)]
              [nextsymbol (get-next-symbol)])
         (cond
           [(not (eq? (car classid) 'ID)) (error 'parser "Illegal class name in new")]
