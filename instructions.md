...
For this and all programming project's, you are to work in groups of up to
...
Only one member of the group has to submit the project.

## Members

Chris Danner and Wolf Mermelstein

# The Language

In this project, you are to create an interpreter for a very
...
`+`, `-`, `*`, `/`, `%` (including the unary `-`), the following comparison
...
type incorrectly (but it is not hard to add the error check).

You do not have to implement short-circuit evaluation of `&&` or `||`, but you
are welcome to do so.

# Challenge

The parser supports nested assignment
...
Here are some more sample programs in this simple language that you can use to
...
certain to write some of your own to fully tests your interpreter.
...
but your naming convention should be consistent.

You should use good style, indentation and proper commenting so that the code
you submit is easy to read and understand.

## Using the Parser

A parser is provided for you called `simpleParser.rkt` Download
...
it is not, you will have to include the path to the file in the load command.

If you are not using racket, you should use `(load "simpleParser.rkt")` at the
top of your file instead of the require function, and you will need to comment
some lines in the `simpleParser.rkt` and `lex.rkt` files.

To parse a program in our simple language, type the program code into a file,
...
statement. The different statements are:
...
an efficient state is unneeded.

What you do have to do is use abstraction to separate your state from the rest
...
the your interpreter can call to manipulate the state.

## Returning a Value

Your interpreter needs to return the proper value.  How you achieve this is up
...
get, and return that value.

## Finally...

Please save your interpreter as a Scheme or Racket file with either the `.scm`,
`.ss`, or `.rkt` extension.
...
- **Performance** (/50)
  ...
  - **No Marks (0 points)**: No work present.
...
- **Abstraction** (/5)
  ...
  - **Overuse of car/cdr/cons (0 points)**: Accessing the state in the "M_"
      functions uses `car` and `cdr` instead of good abstraction.

- **Functional Coding** (/30)
...
- **Return Values** (/10)
  ...
  - **No Marks (0 points)**: Does not return a value or a state.
